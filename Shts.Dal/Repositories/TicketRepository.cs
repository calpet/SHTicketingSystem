    ...
    public class TicketRepository : ITicketRepository
    {
        ...
        public List<TicketDto> GetAll()
        {
            ...
            for (int i = 0; i < _result.Count; i++)
            {
                ...
                {
                    _dto = new TicketDto() { Id = Convert.ToInt32(_result[0]),  Subject = _result[5], Content = _result[6], CreatedAt = Convert.ToDateTime(_result[7]), LastEdited = Convert.ToDateTime(_result[8]), Status = _result[9], Priority = _result[10] };
                    _userDto = new UserDto() { FirstName = _result[1], LastName = _result[2], Role = _result[3], Email = _result[4], Id = Convert.ToInt32(_result[11])};
